...
class WatchDeviceInfo(BaseServiceInfo):
    def __init__(self,service_id, device_count, register_success_count, unregister_count\
        ...
        # self._watch_device_node.set("register_list", str(reduce(lambda x,y: str(x).join(":").join(str(y))\
        #                                                         , self._register_list)))
        # self._watch_device_node.set("unregister_list", str(reduce(lambda x,y: str(x).join(":").join(str(y))\
