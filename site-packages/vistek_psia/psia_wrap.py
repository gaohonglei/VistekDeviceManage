#!/usr/bin/env python
# -*- coding=utf8 -*-
import requests, collections, threading, time, os, sys, logging, logging.handlers, traceback
...
class psia_converter():
    ...
    def to_stream_url(self, device):
        ...
        for channel in self.xml_node.iter('{urn:psialliance-org}StreamingChannel'):
            ...
            rtsp_port_node = transport_node.find('root_ns:rtspPortNo', self.psia_ns)
...
def register_device(device_id, ip, port, user_name, user_pwd):
    ...
    result = request_psia_cmd(url_converter.psia_uri(), url_converter.method(), timeout=1, auth=(user_name, user_pwd))
...
def request_psia_cmd(uri, method, data=None, timeout=None, auth=None):
    ...
    try:
        ...
        logger.error("exception url:{0}".format(uri))
        return None
...
def get_device_status_by_eventlet(device_info):
    if device_info is not None:
        ...
        tmp_out_data = request_psia_cmd(tmp_psia_uri.psia_uri(), tmp_psia_uri.method(), timeout=1, auth=auth)
        if tmp_out_data is None:
            return ("", 0)
        else:
        ...
        try:
            ...
            if response:
                ...
                if device_info_node is not None:
                    syscontact_node = device_info_node.find("{urn:psialliance-org}systemContact")
                    if syscontact_node is not None and "Hikvision" in syscontact_node.text:
                        device.Manufacture = "hikvision"

            if video_out_data is not None:
                if isinstance(video_out_data.text, unicode):
                    root_node = ET.fromstring(video_out_data.text.encode("utf-8"))
                else:
                    root_node = ET.fromstring(video_out_data.text.encode("utf-8"))
                root_namespace = {"root_ns", "urn:psialliance-org"}
                channel_list = []
                for item in root_node.iter("{urn:psialliance-org}VideoInputChannel"):
                    v_channel = v_data.DmDeviceVideoChannel()
                    #id_node = item.find("root_ns:id", root_namespace)
                    id_node = item.find("{urn:psialliance-org}id", root_namespace)
                    if device.DeviceID:
...
class StartGetAllStatusThread(threading.Thread):
    def run(self):
        start_get_all_status()

def start_check_all_staus():
    logger.info("start check all device status.")
...
def start_check_all_status_by_eventlet():
    ...
    while 1:
        if is_stop_event.is_set():
            break
        if not all_status_queue.empty():
            status_list = all_status_queue.get()
            for item in status_list:
                task_pool.spawn(do_check_device_status, item)
